                ...
                if (data.status === 'success' && data.verified) {
                    ...
                    setTimeout(() => {
                        if (data.is_collaboration) {
                            ...
                            if (data.order_id) {
                                window.location.replace(`../actions/order_success.php?order_id=${data.order_id}`);
                            } else {
                                // Fallback to checkout
                                window.location.replace(`../actions/checkout.php?payment=success`);
                            }
                        }
                    }, 1500);
                    ...
                    setTimeout(() => {
                        if (data.order_id) {
                            window.location.href = `../actions/checkout.php?order_id=${data.order_id}&error=verification_failed`;
                        } else {
                            window.location.href = '../actions/checkout.php?error=verification_failed';
                        }
                    }, 5000);
                }
